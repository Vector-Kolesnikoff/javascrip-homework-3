...
var car = {
  engine: 2000,// объем двигателя, числовое поле
  model: 'Lacetti', // модель авто, строка
  name: 'Chevrolet', // бренд авто, строка
  year: 2010, // год выпуска, число
  get used () {
    if (this.year !== yearNow) return('used')
    else return('new')
  },
  set used (value) { if (value === 'new' && this.year < yearNow) this.year = yearNow }, // строка для описания состояния авто, допускаются значения 'used' и 'new'
  info: info
};
